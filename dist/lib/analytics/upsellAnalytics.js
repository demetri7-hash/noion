...
exports.calculateUpsellOpportunities = calculateUpsellOpportunities;
const models_1 = require("../../models");
const mongoose_1 = require("mongoose");
/**
 * Calculate upsell opportunities for a restaurant
 */
async function calculateUpsellOpportunities(restaurantId, options) {
    ...
    for (const [categoryName, config] of Object.entries(patterns)) {
        ...
        transactions.forEach((tx) => {
            const hasPattern = tx.items?.some((item) => config.regex.test(item.name || ''));
            if (hasPattern)
                ordersWithPattern++;
        });
        ...
        if (potentialUpsells > 50 && potentialRevenue > 100) {
            ...
            opportunities.push({
                ...
                actionableInsight: generateActionableInsight(categoryName, currentPenetration, targetPenetration, potentialRevenue, matchingItems[0]?.name || categoryName),
                priority
            });
        }
    }
...
function generateActionableInsight(category, current, target, revenue, topItem) {
