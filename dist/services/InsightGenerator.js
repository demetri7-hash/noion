...
exports.InsightGenerator = exports.insightGenerator = exports.InsightGeneratorService = void 0;
...
class InsightGeneratorService {
     ...
     */
    async generateInsights(restaurantId, startDate, endDate, insightType = Insight_1.InsightType.WEEKLY_SUMMARY) {
        try {
            ...
            const insight = await this.createInsightRecord(restaurant, analysisResult, aiInsights, startDate, endDate, insightType);
     ...
     */
    async analyzeRestaurantData(restaurantId, startDate, endDate) {
     ...
     */
    analyzeRevenueLeakage(transactions) {
        const opportunities = [];
        ...
        const discountedTransactions = transactions.filter(t => t.discounts && t.discounts.length > 0);
        const totalDiscountAmount = discountedTransactions.reduce((sum, t) => sum + t.discounts.reduce((dSum, d) => dSum + d.amount, 0), 0);
        ...
        const avgWaitTime = peakHourTransactions.reduce((sum, t) => sum + (t.timing.waitTime || 0), 0) / peakHourTransactions.length;
     ...
     * Analyze employee performance
     */
    analyzeEmployeePerformance(transactions) {
        ...
        const employeeMetrics = Object.entries(employeeGroups).map(([empId, empTransactions]) => {
            const totalSales = empTransactions.reduce((sum, t) => sum + t.totalAmount, 0);
            const avgTicket = totalSales / empTransactions.length;
            const avgServiceTime = empTransactions.reduce((sum, t) => sum + (t.employee.serviceTime || 0), 0) / empTransactions.length;
            return {
                id: empId,
                ...
                avgTicket,
                avgServiceTime,
                voidRate: empTransactions.filter((t) => t.status === 'voided').length / empTransactions.length
            };
        });
        ...
        const underPerformers = sortedEmployees.slice(-2).map(emp => {
            const issues = [];
            if (emp.avgTicket < 20)
                issues.push('Low average ticket');
            if (emp.avgServiceTime > 10)
                issues.push('Slow service time');
            if (emp.voidRate > 0.05)
                issues.push('High void rate');
            return {
                id: emp.id,
                name: emp.name,
                issues
     ...
     */
    analyzeCustomerExperience(transactions) {
        const avgWaitTime = transactions.reduce((sum, t) => sum + (t.timing.waitTime || 0), 0) / transactions.length;
    ...
    analyzeOperationalEfficiency(transactions) {
        ...
        const avgServiceTime = transactions.reduce((sum, t) => sum + (t.employee.serviceTime || 0), 0) / transactions.length;
        if (avgServiceTime > 8) {
     ...
     */
    analyzeMenuPerformance(transactions) {
        ...
        const underperformingItems = items.filter(item => itemFrequency[item] < avgFrequency * 0.5 && itemRevenue[item] < avgRevenue * 0.5).length;
     ...
     */
    async generateAIInsights(restaurant, analysisResult, insightType) {
     ...
     */
    generateMockClaudeResponse(analysisResult, restaurant) {
        return {
     ...
     */
    buildClaudePrompt(restaurant, analysisResult, insightType) {
...
Focus on actionable, specific advice that will directly impact revenue and customer satisfaction. Use restaurant industry best practices and be precise with financial estimates.`;
    ...
    parseAIResponse(aiResponse, analysisResult) {
        try {
            ...
            const keyFindings = parsed.keyFindings?.map((finding, index) => ({
            ...
            const recommendations = parsed.recommendations?.map((rec, index) => ({
                ...
                steps: rec.steps?.map((step, stepIndex) => ({
     ...
     */
    generateFallbackInsights(analysisResult, processingTime) {
        const keyFindings = [];
    ...
    mapCategory(category) {
        const categoryMap = {
            'revenue_optimization': Insight_1.InsightCategory.REVENUE_OPTIMIZATION,
            'employee_performance': Insight_1.InsightCategory.EMPLOYEE_PERFORMANCE,
            'customer_experience': Insight_1.InsightCategory.CUSTOMER_EXPERIENCE,
            'operational_efficiency': Insight_1.InsightCategory.OPERATIONAL_EFFICIENCY,
            'cost_reduction': Insight_1.InsightCategory.COST_REDUCTION,
            'peak_hour_analysis': Insight_1.InsightCategory.PEAK_HOUR_ANALYSIS,
            'menu_optimization': Insight_1.InsightCategory.MENU_OPTIMIZATION,
            'upselling_opportunities': Insight_1.InsightCategory.UPSELLING_OPPORTUNITIES
        };
        return categoryMap[category.toLowerCase()] || Insight_1.InsightCategory.OPERATIONAL_EFFICIENCY;
    }
    /**
    ...
    mapPriority(priority) {
        const priorityMap = {
            'critical': Insight_1.InsightPriority.CRITICAL,
            'high': Insight_1.InsightPriority.HIGH,
            'medium': Insight_1.InsightPriority.MEDIUM,
            'low': Insight_1.InsightPriority.LOW
        };
        return priorityMap[priority.toLowerCase()] || Insight_1.InsightPriority.MEDIUM;
    }
    /**
     * Create and save insight record
     */
    async createInsightRecord(restaurant, analysisResult, aiInsights, startDate, endDate, insightType) {
        ...
        const insightData = {
            ...
            status: Insight_1.InsightStatus.GENERATED,
        ...
        const insight = new models_1.Insight(insightData);
        return await insight.save();
    }
    /**
     * Calculate industry benchmarks
     */
    calculateBenchmarks(restaurant, analysisResult) {
        const benchmarks = [];
        const restaurantType = restaurant.type.toLowerCase().replace('_', '');
        const industryBenchmark = INDUSTRY_BENCHMARKS.averageTicket[restaurantType] || 25;
        ...
        benchmarks.push({
            ...
            percentileRank: this.calculatePercentileRank(analysisResult.employeePerformance.averageMetrics.avgTicket || 0, industryBenchmark),
            unit: '$'
        });
        return benchmarks;
    }
     ...
     */
    generateInsightTitle(restaurantName, insightType, analysisResult) {
        const revenueAmount = analysisResult.revenueLeakage.total;
        switch (insightType) {
            case Insight_1.InsightType.FREE_DISCOVERY:
                return `${restaurantName}: $${revenueAmount.toFixed(0)} Monthly Revenue Recovery Opportunity`;
            case Insight_1.InsightType.WEEKLY_SUMMARY:
                return `${restaurantName}: Weekly Performance Analysis`;
            case Insight_1.InsightType.MONTHLY_ANALYSIS:
     ...
     */
    calculateOverallPriority(keyFindings) {
        if (keyFindings.some(f => f.priority === Insight_1.InsightPriority.CRITICAL)) {
            return Insight_1.InsightPriority.CRITICAL;
        }
        if (keyFindings.some(f => f.priority === Insight_1.InsightPriority.HIGH)) {
            return Insight_1.InsightPriority.HIGH;
        }
        return Insight_1.InsightPriority.MEDIUM;
    }
}
exports.InsightGeneratorService = InsightGeneratorService;
exports.InsightGenerator = InsightGeneratorService;
// Export singleton instance
exports.insightGenerator = new InsightGeneratorService();
