...
exports.subscriptionService = exports.SubscriptionService = exports.SUBSCRIPTION_PLANS = void 0;
...
class SubscriptionService {
     ...
     */
    async createSubscription(restaurantId, planId, paymentMethodId, billingCycle = 'monthly') {
        try {
            const restaurant = await models_1.Restaurant.findById(restaurantId);
            if (!restaurant) {
                return { success: false, message: 'Restaurant not found' };
            }
            const plan = Object.values(exports.SUBSCRIPTION_PLANS).find(p => p.id === planId);
            if (!plan) {
            ...
            if (subscription.status === 'active') {
                restaurant.status = models_1.RestaurantStatus.ACTIVE;
            }
            await restaurant.save();
     ...
     */
    async updateSubscription(restaurantId, newPlanId, billingCycle) {
        try {
            const restaurant = await models_1.Restaurant.findById(restaurantId);
            if (!restaurant || !restaurant.subscription?.stripeSubscriptionId) {
                return { success: false, message: 'No active subscription found' };
            }
            const newPlan = Object.values(exports.SUBSCRIPTION_PLANS).find(p => p.id === newPlanId);
            if (!newPlan) {
                return { success: false, message: 'Invalid subscription plan' };
            }
     ...
     * Cancel subscription
     */
    async cancelSubscription(restaurantId, immediately = false) {
        try {
            const restaurant = await models_1.Restaurant.findById(restaurantId);
            if (!restaurant || !restaurant.subscription?.stripeSubscriptionId) {
                return { success: false, message: 'No active subscription found' };
            ...
            if (immediately) {
                ...
                restaurant.status = models_1.RestaurantStatus.CHURNED;
            }
            else {
     ...
     */
    async createBillingPortalSession(restaurantId) {
        try {
            const restaurant = await models_1.Restaurant.findById(restaurantId);
    ...
    async handleSubscriptionUpdate(subscription) {
        ...
        if (subscription.status === 'active') {
            ...
            restaurant.status = models_1.RestaurantStatus.SUSPENDED;
        }
        await restaurant.save();
    ...
    async handlePaymentSucceeded(invoice) {
        ...
        if (!restaurant.billing.paymentHistory) {
            restaurant.billing.paymentHistory = [];
        }
        restaurant.billing.paymentHistory.push({
    ...
    async getSubscriptionAnalytics() {
        try {
            const restaurants = await models_1.Restaurant.find({});
            let totalMRR = 0;
            const planCounts = {};
            ...
            restaurants.forEach(restaurant => {
                if (restaurant.subscription?.status === 'active') {
                    activeSubscriptions++;
                    const plan = exports.SUBSCRIPTION_PLANS[restaurant.subscription.plan];
                    if (plan) {
            ...
            const churnedThisMonth = restaurants.filter(r => r.status === 'churned' &&
                r.updatedAt &&
                r.updatedAt > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)).length;
