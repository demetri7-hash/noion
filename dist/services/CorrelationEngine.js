...
const Correlation_1 = __importStar(require("../models/Correlation"));
...
class CorrelationEngine {
     ...
     */
    async discoverCorrelations(restaurantId, startDate, endDate) {
        ...
        const weatherCorrelations = await this.analyzeWeatherCorrelations(restaurantId, transactions);
        result.correlations.push(...weatherCorrelations);
        result.newPatternsFound += weatherCorrelations.length;
        // Analyze event correlations
        const eventCorrelations = await this.analyzeEventCorrelations(restaurantId, transactions);
        ...
        const sportsCorrelations = await this.analyzeSportsCorrelations(restaurantId, transactions);
        result.correlations.push(...sportsCorrelations);
        result.newPatternsFound += sportsCorrelations.length;
        // Analyze holiday correlations
        const holidayCorrelations = await this.analyzeHolidayCorrelations(restaurantId, transactions);
        ...
        const menuCorrelations = await this.analyzeMenuItemWeatherCorrelations(restaurantId, transactions);
        ...
        const multiFactorCorrelations = await this.analyzeMultiFactorPatterns(restaurantId, transactions);
     ...
     */
    async analyzeWeatherCorrelations(restaurantId, transactions) {
        const correlations = [];
        try {
            // Get restaurant location for weather API
            const Restaurant = (await Promise.resolve().then(() => __importStar(require('../models/Restaurant')))).default;
            ...
            if (tempCorrelation && Math.abs(tempCorrelation.statistics.correlation) > 0.15) {
                correlations.push(await this.createCorrelation({
                    restaurantId,
                    type: Correlation_1.CorrelationType.WEATHER_SALES,
                    factor: {
                        type: 'weather',
     ...
     */
    async calculateTemperatureSalesCorrelation(transactions, restaurantLocation) {
        ...
        for (const [dateKey, dayData] of Array.from(dailyRevenue.entries())) {
            try {
                ...
                let weather = await ExternalDataService_1.weatherService.getHistoricalWeather(restaurantLocation.lat, restaurantLocation.lon, timestamp);
        ...
        const stats = this.calculatePearsonCorrelation(dailyData.map(d => d.temp), dailyData.map(d => d.revenue));
        ...
        return {
            ...
            pattern: {
                ...
                recommendation: Math.abs(stats.correlation) > 0.5
     ...
     */
    async calculatePrecipitationSalesCorrelation(transactions, restaurantLocation) {
        const dailyRevenue = new Map();
        transactions.forEach(t => {
            const dateKey = t.transactionDate.toISOString().split('T')[0];
            const existing = dailyRevenue.get(dateKey);
     ...
     */
    async calculateWeatherCategorySalesCorrelation(transactions, restaurantLocation) {
        const dailyRevenue = new Map();
        transactions.forEach(t => {
            const dateKey = t.transactionDate.toISOString().split('T')[0];
            const existing = dailyRevenue.get(dateKey);
     ...
     */
    async analyzeEventCorrelations(restaurantId, transactions) {
        const correlations = [];
        try {
            ...
            const nonEventDays = [];
            for (const [dateKey, dayData] of Array.from(dailyRevenue.entries())) {
                const events = await ExternalDataService_1.eventsService.getMajorEvents(location.lat, location.lon, dayData.date);
                // Filter for high impact events
                const majorEvents = events.filter(e => e.impactLevel === 'high' || e.impactLevel === 'critical');
            ...
            if (Math.abs(change) > 15) {
                ...
                const correlation = await this.createCorrelation({
                    restaurantId,
                    type: Correlation_1.CorrelationType.EVENTS_TRAFFIC,
                    factor: {
                        type: 'event',
                    ...
                    pattern: {
                        ...
                        thenOutcome: `Revenue ${change > 0 ? 'increases' : 'decreases'} by ${Math.abs(change).toFixed(1)}%`,
                        strength: Math.abs(change) > 35 ? Correlation_1.CorrelationStrength.STRONG :
                            Math.abs(change) > 20 ? Correlation_1.CorrelationStrength.MODERATE : Correlation_1.CorrelationStrength.WEAK,
                        actionable: true,
                        recommendation: change > 0
     ...
     * Analyze sports game correlations
     */
    async analyzeSportsCorrelations(restaurantId, transactions) {
        const correlations = [];
        try {
            // Get restaurant location
            const Restaurant = (await Promise.resolve().then(() => __importStar(require('../models/Restaurant')))).default;
            ...
            const nonGameDays = [];
            for (const [dateKey, dayData] of Array.from(dailyRevenue.entries())) {
                const games = await ExternalDataService_1.sportsService.getGamesOnDate(dayData.date, location.lat, location.lon, 30 // 30 mile radius
                );
                // Filter for high/critical impact games
                const majorGames = games.filter(g => g.impactLevel === 'high' || g.impactLevel === 'critical');
            ...
            if (Math.abs(change) > 15) {
                ...
                const correlation = await this.createCorrelation({
                    restaurantId,
                    type: Correlation_1.CorrelationType.EVENTS_TRAFFIC, // Using existing type
                    factor: {
                        type: 'sports',
                    ...
                    pattern: {
                        ...
                        thenOutcome: `Revenue ${change > 0 ? 'increases' : 'decreases'} by ${Math.abs(change).toFixed(1)}%`,
                        strength: Math.abs(change) > 40 ? Correlation_1.CorrelationStrength.STRONG :
                            Math.abs(change) > 25 ? Correlation_1.CorrelationStrength.MODERATE : Correlation_1.CorrelationStrength.WEAK,
                        actionable: true,
                        recommendation: change > 0
     ...
     * Analyze holiday correlations
     */
    async analyzeHolidayCorrelations(restaurantId, transactions) {
        const correlations = [];
        // Group transactions by holiday vs non-holiday
        const holidayTransactions = [];
        const normalTransactions = [];
        transactions.forEach(t => {
            const holiday = ExternalDataService_1.holidayService.getHoliday(t.transactionDate);
            if (holiday) {
        ...
        const holidayAvg = holidayTransactions.reduce((sum, t) => sum + t.totalAmount, 0) /
            holidayTransactions.length;
        const normalAvg = normalTransactions.reduce((sum, t) => sum + t.totalAmount, 0) /
        ...
        if (Math.abs(change) > 10) {
            ...
            const correlation = await this.createCorrelation({
                restaurantId,
                type: Correlation_1.CorrelationType.HOLIDAY_SALES,
                factor: {
                    type: 'holiday',
                ...
                pattern: {
                    ...
                    thenOutcome: `Revenue ${change > 0 ? 'increases' : 'decreases'} by ${Math.abs(change).toFixed(1)}%`,
                    strength: Math.abs(change) > 30 ? Correlation_1.CorrelationStrength.STRONG : Correlation_1.CorrelationStrength.MODERATE,
                    actionable: true,
                    recommendation: change > 0
     ...
     */
    async analyzeMenuItemWeatherCorrelations(restaurantId, transactions) {
        const correlations = [];
        try {
            // Get restaurant location
            const Restaurant = (await Promise.resolve().then(() => __importStar(require('../models/Restaurant')))).default;
            ...
            for (const [itemCategory, weatherData] of Object.entries(itemsByWeather)) {
                ...
                for (const itemName of allItems) {
                    ...
                    if (totalSales < 10)
                        continue; // Need minimum sales data
                    ...
                    if (correlationType) {
                        ...
                        const correlation = await this.createCorrelation({
                            ...
                            pattern: {
                                ...
                                strength: percentage > 70 ? Correlation_1.CorrelationStrength.STRONG :
                                    percentage > 60 ? Correlation_1.CorrelationStrength.MODERATE : Correlation_1.CorrelationStrength.WEAK,
                                actionable: true,
     ...
     */
    getMenuItemRecommendation(itemName, category, weather) {
     ...
     */
    async analyzeMultiFactorPatterns(restaurantId, transactions) {
        const correlations = [];
        try {
            // Get restaurant location
            const Restaurant = (await Promise.resolve().then(() => __importStar(require('../models/Restaurant')))).default;
            ...
            for (const [dateKey, dayData] of Array.from(dailyData.entries())) {
                ...
                const games = await ExternalDataService_1.sportsService.getGamesOnDate(date, location.lat, location.lon, 30);
                const hasSportsGame = games.some(g => g.impactLevel === 'high' || g.impactLevel === 'critical');
                // Check for events
                const events = await ExternalDataService_1.eventsService.getMajorEvents(location.lat, location.lon, date);
            ...
            const weekendGoodWeatherEvent = scenarios.filter(s => s.isWeekend &&
                s.weather.temp >= 65 && s.weather.temp <= 85 &&
                s.weather.condition === 'Clear' &&
                (s.hasSportsGame || s.hasEvent));
            const normalDays = scenarios.filter(s => !s.isWeekend &&
                !s.hasSportsGame &&
                !s.hasEvent &&
                ...
                if (Math.abs(change) > 25) {
                    const correlation = await this.createCorrelation({
                        ...
                        pattern: {
                            ...
                            strength: Math.abs(change) > 50 ? Correlation_1.CorrelationStrength.VERY_STRONG :
                                Math.abs(change) > 35 ? Correlation_1.CorrelationStrength.STRONG : Correlation_1.CorrelationStrength.MODERATE,
                            actionable: true,
                            recommendation: change > 0
            ...
            const rainyFridayNoEvents = scenarios.filter(s => s.dayOfWeek === 'Friday' &&
                s.weather.condition === 'Rain' &&
                !s.hasSportsGame &&
                !s.hasEvent);
            const normalFridays = scenarios.filter(s => s.dayOfWeek === 'Friday' &&
                s.weather.condition !== 'Rain');
                ...
                if (Math.abs(change) > 15) {
                    const correlation = await this.createCorrelation({
                        ...
                        pattern: {
                            ...
                            strength: Math.abs(change) > 25 ? Correlation_1.CorrelationStrength.STRONG : Correlation_1.CorrelationStrength.MODERATE,
                            actionable: true,
                            recommendation: change < 0
            ...
            const mondayCold = scenarios.filter(s => s.dayOfWeek === 'Monday' &&
                s.weather.temp < 50);
            const mondayNormal = scenarios.filter(s => s.dayOfWeek === 'Monday' &&
                ...
                if (Math.abs(change) > 12) {
                    const correlation = await this.createCorrelation({
                        ...
                        pattern: {
                            ...
                            strength: Math.abs(change) > 20 ? Correlation_1.CorrelationStrength.MODERATE : Correlation_1.CorrelationStrength.WEAK,
                            actionable: true,
                            recommendation: change > 0
     ...
     */
    async validateExistingPatterns(restaurantId, transactions) {
        let validated = 0;
        let invalidated = 0;
        // Get existing patterns for this restaurant
        const existingPatterns = await Correlation_1.default.find({
            $or: [
                { restaurantId: new mongoose_1.Types.ObjectId(restaurantId) },
                { scope: 'global' },
                { scope: 'regional' }
        ...
        for (const pattern of existingPatterns) {
            ...
            await pattern.updateValidation(isValid);
            if (isValid)
                validated++;
            else
                invalidated++;
     ...
     */
    async testPattern(pattern, transactions) {
    ...
    async predict(input) {
        ...
        const patterns = await Correlation_1.default.findForRestaurant(input.restaurantId, 'northeast', // In production, get from restaurant location
        ...
        for (const pattern of patterns) {
            if (!pattern.isActive || pattern.confidence < 60)
                continue;
     ...
     */
    applyPattern(pattern, input, baseline) {
    ...
    combinePredictions(predictions) {
        ...
        byMetric.forEach((preds, metric) => {
            ...
            const weightedValue = preds.reduce((sum, p) => sum + p.predictedValue * (p.confidence / totalConfidence), 0);
     ...
     */
    async createCorrelation(params) {
        const correlation = new Correlation_1.default({
            scope: 'restaurant',
            restaurantId: new mongoose_1.Types.ObjectId(params.restaurantId),
            type: params.type,
     ...
     */
    classifyStrength(correlation) {
        const abs = Math.abs(correlation);
        if (abs >= 0.8)
            return Correlation_1.CorrelationStrength.VERY_STRONG;
        if (abs >= 0.6)
            return Correlation_1.CorrelationStrength.STRONG;
        if (abs >= 0.4)
            return Correlation_1.CorrelationStrength.MODERATE;
        if (abs >= 0.2)
            return Correlation_1.CorrelationStrength.WEAK;
        if (abs > 0)
            return Correlation_1.CorrelationStrength.VERY_WEAK;
        return Correlation_1.CorrelationStrength.NONE;
    }
    /**
     * Contribute restaurant data to global learning
     */
    async contributeToGlobalLearning(restaurantId) {
        // Get restaurant-specific correlations
        const restaurantCorrelations = await Correlation_1.default.find({
            restaurantId: new mongoose_1.Types.ObjectId(restaurantId),
        ...
        for (const resCor of restaurantCorrelations) {
            ...
            const globalPattern = await Correlation_1.default.findOne({
                scope: 'global',
                type: resCor.type,
            ...
            if (globalPattern) {
                ...
                const globalCor = new Correlation_1.default({
                    ...resCor.toObject(),
                    _id: undefined,
                    scope: 'global',
     ...
     */
    generateRealisticSeasonalWeather(date, location) {
