...
exports.menuAnalyticsService = exports.MenuAnalyticsService = void 0;
...
class MenuAnalyticsService {
     ...
     */
    async analyzeMenu(restaurantId, startDate, endDate) {
        ...
        const upsellOpportunities = await this.findUpsellOpportunities(transactions, menuItems, itemPairs);
     ...
     */
    extractMenuPatterns(transactions) {
     ...
     */
    async findUpsellOpportunities(transactions, menuItems, itemPairs) {
        const opportunities = [];
        ...
        for (const [itemName, item] of Array.from(menuItems.entries())) {
            ...
            for (const companion of companionArray.slice(0, 3)) {
                const opportunity = await this.createUpsellOpportunity(item, companion.item, companion.attachRate, transactions, item.frequency);
                opportunities.push(opportunity);
            }
        }
        // Sort by revenue potential
        return opportunities.sort((a, b) => b.revenueImpact.missedMonthly - a.revenueImpact.missedMonthly).slice(0, 10); // Top 10 opportunities
    }
    /**
     * Create detailed upsell opportunity
     */
    async createUpsellOpportunity(baseItem, suggestedItem, attachRate, transactions, baseItemFrequency) {
        // Count orders with base item
        const ordersWithBase = transactions.filter(t => t.items.some((item) => this.normalizeItemName(item.name) === this.normalizeItemName(baseItem.name)));
        // Count how many of those also have the suggested item
        const ordersWithBoth = ordersWithBase.filter(t => t.items.some((item) => this.normalizeItemName(item.name) === this.normalizeItemName(suggestedItem.name)));
        ...
        const recommendation = this.createRecommendation(baseItem, suggestedItem, attachRate, potentialAt50, potentialAt75);
        return {
            baseItem,
            suggestedItem,
            pattern: {
     ...
     */
    createRecommendation(baseItem, suggestedItem, attachRate, revenueAt50, revenueAt75) {
...
- At 75% success rate: +$${revenueAt75.toFixed(0)}/month
     ...
     */
    findBundleOpportunities(itemPairs, menuItems) {
        const bundles = [];
            ...
            for (let i = 0; i < companionArray.length; i++) {
                for (let j = i + 1; j < companionArray.length; j++) {
                    ...
                    if (comp2Count >= 5) {
                        ...
                        const frequency = Math.min(companions.get(comp1.name) || 0, companions.get(comp2.name) || 0, comp2Count);
                        bundles.push({
                            items,
                            frequency,
                            bundlePrice,
     ...
     */
    getUnderperformingItems(menuItems, topItems) {
     ...
     */
    async generateMenuReport(restaurantId) {
        const endDate = new Date();
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - 30);
        const insights = await this.analyzeMenu(restaurantId, startDate, endDate);
        const totalPotential = insights.upsellOpportunities.reduce((sum, opp) => sum + opp.revenueImpact.ifUpsoldAt50Percent, 0);
     ...
     */
    generateSummary(insights, totalPotential) {
...
exports.MenuAnalyticsService = MenuAnalyticsService;
// Export singleton
exports.menuAnalyticsService = new MenuAnalyticsService();
