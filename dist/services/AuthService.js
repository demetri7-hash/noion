...
exports.authService = exports.AuthService = exports.UserRole = void 0;
...
class AuthService {
    ...
    async registerRestaurantOwner(userData) {
        try {
            ...
            const existingRestaurant = await models_1.Restaurant.findOne({
    ...
    async login(email, password) {
        try {
            ...
            if (this.isAccountLocked(restaurant.owner)) {
                const lockoutTime = Math.ceil(((restaurant.owner.lockoutUntil.getTime() - Date.now()) / 1000 / 60));
                return {
                    success: false,
            ...
            // Verify password
            const isValidPassword = await this.verifyPassword(password, restaurant.owner.password);
            if (!isValidPassword) {
            ...
            const { token, refreshToken } = this.generateTokens({
                userId: restaurant.owner.email,
                restaurantId: String(restaurant._id),
                role: restaurant.owner.role || UserRole.RESTAURANT_OWNER,
                email: restaurant.owner.email
            });
            // Remove password from response
            const userResponse = restaurant.toObject();
    ...
    async refreshToken(refreshToken) {
        try {
            ...
            const restaurant = await models_1.Restaurant.findById(decoded.restaurantId);
            if (!restaurant) {
    ...
    generateTokens(payload) {
        ...
        const refreshToken = jwt.sign(payload, AUTH_CONFIG.jwtSecret);
    ...
    async hashPassword(password) {
        return bcryptjs_1.default.hash(password, AUTH_CONFIG.saltRounds);
    }
    /**
     * Verify password
     */
     ...
     */
    async handleSuccessfulLogin(restaurant) {
