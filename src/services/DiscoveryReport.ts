import { Restaurant, Insight, IInsight } from '../models';
import { InsightType, InsightStatus } from '../models/Insight';
...
// Interface for discovery report data
interface IDiscoveryReportData {
  restaurant: any;
  insight: IInsight;
  lostRevenue: {
    total: number;
...
export class DiscoveryReportService {
   ...
   */
  async generateDiscoveryReport(restaurantId: string): Promise<IDiscoveryReportData> {
    try {
   ...
   */
  private async validateRestaurantEligibility(restaurantId: string): Promise<any> {
   ...
   */
  private async generateDiscoveryAnalysis(restaurant: any): Promise<IInsight> {
    const endDate = new Date();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - DISCOVERY_REPORT_CONFIG.analysisperiodDays);

    // Generate free discovery insight
    const insight = await insightGenerator.generateInsights(
      restaurant._id.toString(),
      startDate,
      endDate,
      InsightType.FREE_DISCOVERY
    );
   ...
   */
  private async enhanceRevenueAnalysis(restaurant: any, insight: IInsight): Promise<any> {
   ...
   */
  private async processDiscoveryData(restaurant: any, insight: IInsight): Promise<IDiscoveryReportData> {
    ...
    const quickWins = insight.recommendations
      ...
      .map(rec => ({
        ...
        difficulty: rec.implementation.difficulty as 'Easy' | 'Medium' | 'Hard'
      }));
   ...
   */
  private calculateIndustryComparison(restaurant: any, insight: IInsight): IDiscoveryReportData['industryComparison'] {
    ...
    let performance: 'Below Average' | 'Average' | 'Above Average';
    if (avgPercentile >= 75) performance = 'Above Average';
    else if (avgPercentile >= 40) performance = 'Average';
   ...
   * Send discovery report email
   */
  private async sendDiscoveryReportEmail(reportData: IDiscoveryReportData): Promise<void> {
    try {
      // Generate PDF report
   ...
   * Generate PDF report
   */
  private async generateDiscoveryReportPDF(reportData: IDiscoveryReportData): Promise<Buffer> {
...
TOP OPPORTUNITIES:
${reportData.lostRevenue.breakdown.slice(0, 3).map(item => 
  `• ${item.category}: $${item.amount.toFixed(0)} (${item.percentage}%)`
).join('\n')}

QUICK WINS (30-Day Implementation):
${reportData.quickWins.map(win => 
  `• ${win.action}: $${win.impact.toFixed(0)} potential impact`
).join('\n')}
...
Ready to recover this revenue? Schedule your free consultation at noionanalytics.com/consultation
   ...
   */
  private async updateRestaurantDiscoveryStatus(restaurant: any, insight: IInsight): Promise<void> {
    ...
    insight.status = InsightStatus.SENT;
   ...
   */
  async scheduleFollowUpSequence(restaurantId: string): Promise<void> {
    const restaurant = await Restaurant.findById(restaurantId);
   ...
   */
  async trackReportEngagement(restaurantId: string, eventType: 'email_opened' | 'report_viewed' | 'consultation_booked'): Promise<void> {
    try {
      // Find the discovery insight
      const insight = await Insight.findOne({
        restaurantId: restaurantId,
        type: InsightType.FREE_DISCOVERY
      }).sort({ createdAt: -1 });
   ...
   */
  async isEligibleForDiscoveryReport(restaurantId: string): Promise<{
    eligible: boolean;
    reason?: string;
   ...
   */
  async getDiscoveryReportAnalytics(): Promise<{
    totalReportsSent: number;
    conversionRate: number;
    averageLostRevenue: number;
    topOpportunities: Array<{ category: string; frequency: number; avgAmount: number }>;
  }> {
    try {
      // Get all discovery reports
      const discoveryInsights = await Insight.find({
        type: InsightType.FREE_DISCOVERY
      });
      ...
      const conversions = discoveryInsights.filter(insight => 
        insight.status === InsightStatus.ACTED_UPON
      ).length;
      const conversionRate = totalReportsSent > 0 ? (conversions / totalReportsSent) * 100 : 0;

      // Calculate average lost revenue
      const totalLostRevenue = discoveryInsights.reduce((sum, insight) => 
        sum + insight.lostRevenue.total, 0
      );
      const averageLostRevenue = totalReportsSent > 0 ? totalLostRevenue / totalReportsSent : 0;

      // Find top opportunities
      const opportunityMap: { [key: string]: { count: number; totalAmount: number } } = {};
...
export const discoveryReportService = new DiscoveryReportService();
export { DiscoveryReportService as DiscoveryReport };